...
| `image-patch/` | Read-only bind-mount overrides (coalescer + kernel backports + optional [ABLATE](#ablite) hook) |
| `files/` | Bundled refusal direction for the optional [ABLATE](#ablite) path (MIT-licensed; see `files/README.md`) |
...
| `ABLATE` | 0 | **opt-in** refusal-direction ablation ([details](#ablite)) — `1` projects a published refusal direction out of the attention output stream at runtime. No weight edits, no measurable perf cost, fully inert while off |
...
**Setup** — a default direction ships with this repo at `files/direction_r1.pt` (MIT-licensed capture from [drowzeys' abliterated release](https://github.com/drowzeys/DeepSeek-V4-Flash-DSpark-Abliterated-Uncensored-1M-57toks); attribution in `files/README.md`). The first `ABLATE=1` boot auto-provisions it into `./data/files/` (container path `/models/files/`). To use your own direction instead, drop a unit-norm 4096-dim `.pt` at `./data/files/direction_r1.pt` or point `DSV4_ABLATE_FILE` at it — a pre-existing `data/files/` file is never overwritten. Then:

```bash
ABLATE=1 ./start.sh          # one boot with ablation (or edit the ABLATE default in start.sh)
```
...
- Lives in `image-patch/vllm/models/deepseek_v4/nvidia/model.py` — the image's file plus an additive, env-gated hook, bind-mounted read-only like the other `image-patch/` overrides (remove that mount in `start.sh` to return fully stock). Plain tensor math — torch.compile- and CUDA-graph-capturable.
- Validated in-container by `image-patch/selftest_ablate.py`: env parsing, unit-norm load, layer gating (incl. draft exclusion), fp32 projection identity `y·v = (1−λ)·x·v`, disabled-layer passthrough.
